...
const AppSidebar: React.FC<AppSidebarProps> = ({ 
    ...
    chapters = [], currentChapterId, onChapterSelect, onAutoGenerate, onDeleteChapter
}) => {
  ...
  const handleDeleteChapterClick = (e: React.MouseEvent, id: number) => {
      e.stopPropagation();
      e.preventDefault();
      if (onDeleteChapter) {
          if (window.confirm(`确认删除第 ${id} 章?`)) {
              onDeleteChapter(id);
          }
      }
  };
                    ...
                    <div className="space-y-0.5 ml-1">
                        {group.chapters.map(chapter => (
                            <div
                                ...
                                className={`group px-3 py-1.5 rounded-md text-xs cursor-pointer truncate transition-colors flex items-center justify-between relative pr-14 ${
                            ...
                            >
                                ...
                                <div className="flex items-center absolute right-1 gap-1">
                                    {chapter.wordCount ? (
                                    ...
                                    {chapter.summary && (
                                        <button
                                            ...
                                            className={`p-1 rounded transition-all hover:bg-gray-700 hover:text-white ${
                                    ...
                                    {onDeleteChapter && (
                                        <button
                                            ...
                                            className="p-1 rounded transition-all hover:bg-red-900/50 hover:text-red-400 opacity-0 group-hover:opacity-100 text-gray-500"
                                            title="删除章节"
                                        >
                                            <Trash2 size={10} />
                                        </button>
  ...
  return (
    <div className="w-64 bg-gray-900 text-gray-300 flex flex-col h-full border-r border-gray-800 shrink-0 transition-all duration-300 font-sans">
      ...
      <div className="p-4 border-t border-gray-800 text-[10px] text-gray-600 text-center flex justify-between items-center">
         <span>v1.7.0</span>
         <span className="flex items-center gap-1 opacity-50">Chapters Delete</span>
      </div>
    </div>
  );
};
